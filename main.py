...
def calcul_arie_dreptunghi(a, b, c):
...
numere = [1, 7, 3, 9, 2]
def gaseste_numar_maxim(numere):
...
numere = [1, 2, 3, 4, 5, 6]
def filtreaza_numere_pare(numere):
  for i in range(numere):
    if i / 2:
      numere.pop(i)
  print(numere)
...
def afiseaza_informatii_student():
...
def adauga_nota(student, nota):
  if "note" not in student:
    student["note"] = []
  student["note"].append(int(nota))

adauga_nota(student1, 10)
adauga_nota(student2, 8)
adauga_nota(student1, 9)
...
def numara_studenti_promovati(b):
  for i in range(b):
    promoted = 0
    if i["medie"] >= 5:
      promoted += 1
...
numara_studenti_promovati(lista_studenti_exemplu)
...
def calculeaza_total_cos(dict, list):
  a = 0
  for name, cost in dict.items():
    if name in list:
      a += cost
...
def adauga_contact(agenda, nume, numar):
  agenda[nume] = []
  agenda[nume].append(numar)


def cauta_contact(agenda, numar_cautat):
  for nume in agenda:
    if numar_cautat in agenda[nume]:
      print("Găsit:", nume)
...
def afiseaza_toate_contactele(agenda):
  for name, number in range(agenda):
    print(name+":", number)
